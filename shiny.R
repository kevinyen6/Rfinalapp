...
myRtl<- "https://raw.githubusercontent.com/kevinyen6/Rfinalapp/master/dataset.csv"
...
parameterFunction <- function(Tp,Sd,Cr,Gc,Ts,Hh,Hs,Dy,Ar){
  ...
  Ar <- as.numeric(Ar) # 0 for no preference, 1 for good air quality preferred
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("var9", "0 for no preference, 1 for good air quality preferred",
                  min = 0, max = 1,
                  value = 0, step = 1)
    ...
    mainPanel(
      tabsetPanel(type = "tabs",
                  tabPanel("Summary & Map", plotOutput("map"), verbatimTextOutput("summary")),
...
server <- function(input, output) {
  ...
  output$map <- renderPlot({
    map(database = "state")
    c = c(as.character(datainput()$State))
